...
def extractFemaleNames(filename,letter):
  ...
  """
  # +++your code here+++
  Local_FemaleNames_RankList=[]
  f=open(filename,'r'); #in read mode
	#this list of tuple has the female names and rank 
  Rank_FemaleNames_List_of_Tupels=re.findall(r'<tr.+<td>(\d+).+<td>(?:[a-zA-Z]+).+<td>('+letter+'[a-zA-Z]+)',f.read());	
  #print (Rank_FemaleNames_List_of_Tupels)
  for  a_tupel in  Rank_FemaleNames_List_of_Tupels:
    Local_FemaleNames_RankList.append(a_tupel[1] +' '+a_tupel[0])
    #print a_tupel
  Local_FemaleNames_RankList.sort()
  f.close()
  return(Local_FemaleNames_RankList)
...
def extractMaleNames(filename,letter):
  ...
  """
  # +++your code here+++
  #<tr.+<td>(\d+).+<td>([a-zA-Z]+).+<td>(?:[a-zA-Z]+)
  Local_maleNames_RankList=[]
  f=open(filename,'r'); #in read mode
	#this list of tuple has the female names and rank 
  Rank_maleNames_List_of_Tupels=re.findall(r'<tr.+<td>(\d+).+<td>('+letter+'[a-zA-Z]+).+<td>(?:[a-zA-Z]+)',f.read());	
  #print (Rank_FemaleNames_List_of_Tupels)
  for  a_tupel in  Rank_maleNames_List_of_Tupels:
    Local_maleNames_RankList.append(a_tupel[1]+' '+a_tupel[0])
    #print a_tupel
  Local_maleNames_RankList.sort(reverse=True)
  f.close()
  return(Local_maleNames_RankList)
  #print(Local_maleNames_RankList)
...
def extractDocAndPopularityDate(filename):
  ...
  documentdate=re.findall(r'<h1>.+</h1>(.+)</td>',filehandle.read())
  filehandle.close()
  filehandle=open(filename,'r')
  popdate=re.findall(r'<h.+?>(.+)</h3>',filehandle.read())
  filehandle.close()
  return documentdate[0]+' and '+popdate[0]
